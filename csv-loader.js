...
class CSVLoader {
  static async loadQuestions(csvPath = 'questions.csv') {
    try {
      ...
      console.log('CSV chargé avec succès, longueur:', csvText.length, 'caractères');
      const result = this.parseCSV(csvText);
      console.log('Questions parsées:', result.dimensions.reduce((total, dim) => total + dim.questions.length, 0));
      return result;
    } catch (error) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const question = {
        ...
        whyContent: {
          ...
          whenDoneWell: row.when_done_well ? row.when_done_well.split('|').map(s => s.trim()) : [],
          problemsWithout: row.problems_without ? row.problems_without.split('|').map(s => s.trim()) : [],
          realExample: row.real_example || null
        }
      };
