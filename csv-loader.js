// csv-loader.js - CSV parser and data loader (FIXED VERSION)
...
class CSVLoader {
  ...
  static getEmbeddedCSV() {
    // PROPERLY FORMATTED CSV with quoted fields containing commas
...
improvement,CONTINUOUS IMPROVEMENT,How well does the pod learn and evolve their practices?,36,Improvements and their impacts are celebrated and shared,A,,,,,,,,,,,,,,"Uncelebrated improvements die quietly. Recognition fuels more improvement, sharing spreads benefits, and celebration builds culture. Success needs spotlights.","Improvement becomes cultural norm|Knowledge spreads across organization|Team pride and motivation high|Positive reinforcement cycle","Improvements forgotten and abandoned|Lessons learned stay local|Team feels unappreciated|Innovation dies from neglect"`;
  ...
  static parseCSV(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length !== headers.length) {
        console.warn(`Line ${i + 1}: Expected ${headers.length} columns but got ${values.length}`);
        if (values.length < 22) {  // Minimum needed for a complete row with why content
          console.error(`Line ${i + 1} is incomplete, skipping`);
          continue;
        }
      }
      ...
      if (!row.dimension_id || !row.question_id || !row.question_text || !row.question_type) {
        console.error(`Line ${i + 1}: Missing essential fields, skipping`);
        continue;
      }
      ...
      console.log(`Added question ${question.id}: ${question.text.substring(0, 50)}...`);
    }
  ...
  // IMPROVED CSV line parser that handles quotes properly
